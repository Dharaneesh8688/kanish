...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt->bind_param(
        'isisssssddssddddi',
        $client_id, $client_name_text, $client_phone_id, $client_phone_text, $doc_no, $doc_date, $due_date, $payment_terms,
        $shipping_charges, $discount_percent, $items_json,
        $subtotal, $cgst, $sgst, $igst, $grand_total,
        $id
    );

    if ($stmt->execute()) {
        header('Location: quotation.php?updated=1');
        exit;
    } else {
...
if (!$quotation) {
    ...
    <div class="alert alert-danger">Quotation not found.</div>
    </div>
    ');
}
...
<div class="main-content">
  <div class="container-fluid py-4">
    <div class="bg-white shadow-sm rounded p-4">
      ...
      <p class="text-muted mb-4">edit Quotation</p>

      <form id="quotation_form" method="POST" action="edit_quotation.php?id=<?php echo $id; ?>">
        <!-- Quotation Info -->
...
<input type="hidden" name="client_phone_text" id="client_phone_text" >
  ...
  <option value="<?php echo htmlspecialchars($quotation['client_id']); ?>" selected>
    <?php echo htmlspecialchars($quotation['client_name']); ?>
          ...
          <div class="col-md-3">
            ...
            <select name="client_name" id="client_name" class="form-select" required>
  <option value="<?php echo htmlspecialchars($quotation['client_id']); ?>" selected>
    <?php echo htmlspecialchars($quotation['client_phone']); ?>
  </option>
</select>
          </div>
          <div class="col-md-2">
...
?>
      ...
      <div class="row">
        ...
        <div class="text-end">
          ...
          <a href="quotation.php" class="btn btn-danger">Cancel</a>
        </div>
...
$('#quotation_form').on('submit', function(e){
  ...
  // Remove this submit handler to avoid recursion
  $('#quotation_form').off('submit');
...
<script>
          function updateSubTotal() {
            ...
            $('#particular_table tbody tr').each(function () {
              const total = parseFloat($(this).find('td').eq(9).text()) || 0;
              const taxPercent = parseFloat($(this).find('td').eq(8).text()) || 0;
...
function calculateArea() {
            const width = parseFloat($('#item_width').val()) || 0;
            const height = parseFloat($('#item_height').val()) || 0;
