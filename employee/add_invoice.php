...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    (
      client_id, client_name, client_phone_id, client_phone,
      doc_no, doc_date, due_date, payment_terms,
      shipping_charges, discount_percent, items_json,
      subtotal, cgst, sgst, igst, created_by, grand_total
    )
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    
    $stmt->bind_param(
      'isisssssddssdddsd',
      $client_id, $client_name_text, $client_phone_id, $client_phone_text,
      $doc_no, $doc_date, $due_date, $payment_terms,
      $shipping_charges, $discount_percent, $items_json,
      $subtotal, $cgst, $sgst, $igst, $created_by, $grand_total
    );
